...
mod tests {
    ...
    fn prefix_iter_with_byte_255() {
        ...
        db.put(&mut wtxn, &[0, 0, 0, 254, 119, 111, 114, 108, 100], b"world").unwrap();
        db.put(&mut wtxn, &[0, 0, 0, 255, 104, 101, 108, 108, 111], b"hello").unwrap();
        db.put(&mut wtxn, &[0, 0, 0, 255, 119, 111, 114, 108, 100], b"world").unwrap();
        db.put(&mut wtxn, &[0, 0, 1, 0, 119, 111, 114, 108, 100], b"world").unwrap();
        ...
        assert_eq!(
            ...
            Some((&[0u8, 0, 0, 255, 104, 101, 108, 108, 111][..], &b"hello"[..]))
        );
        assert_eq!(
            iter.next().transpose().unwrap(),
    ...
    fn range_iter_last() {
        ...
        let mut iter = db.range(&wtxn, 2_i32.to_be_bytes()..=4_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.next().transpose().unwrap(), Some((&2_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.last().transpose().unwrap(), Some((&4_i32.to_be_bytes()[..], &[][..])));

        let mut iter = db.range(&wtxn, 2_i32.to_be_bytes()..4_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.next().transpose().unwrap(), Some((&2_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.last().transpose().unwrap(), Some((&3_i32.to_be_bytes()[..], &[][..])));
        ...
        let iter = db.range(&wtxn, 2_i32.to_be_bytes()..2_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.last().transpose().unwrap(), None);

        let iter = db.range(&wtxn, 2_i32.to_be_bytes()..=1_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.last().transpose().unwrap(), None);
    ...
    fn rev_range_iter_last() {
        ...
        // Lets check that we properly get the last entry.
        let iter = db.rev_range(&wtxn, 1_i32.to_be_bytes()..=3_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.last().transpose().unwrap(), Some((&1_i32.to_be_bytes()[..], &[][..])));

        let mut iter = db.rev_range(&wtxn, 0_i32.to_be_bytes()..4_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.next().transpose().unwrap(), Some((&3_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.next().transpose().unwrap(), Some((&2_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.last().transpose().unwrap(), Some((&1_i32.to_be_bytes()[..], &[][..])));

        let mut iter = db.rev_range(&wtxn, 0_i32.to_be_bytes()..=5_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.next().transpose().unwrap(), Some((&4_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.next().transpose().unwrap(), Some((&3_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.next().transpose().unwrap(), Some((&2_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.next().transpose().unwrap(), Some((&1_i32.to_be_bytes()[..], &[][..])));
        ...
        let iter = db.rev_range(&wtxn, 0_i32.to_be_bytes()..=5_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.last().transpose().unwrap(), Some((&1_i32.to_be_bytes()[..], &[][..])));

        let mut iter = db.rev_range(&wtxn, 4_i32.to_be_bytes()..=4_i32.to_be_bytes()).unwrap();
        assert_eq!(iter.next().transpose().unwrap(), Some((&4_i32.to_be_bytes()[..], &[][..])));
        assert_eq!(iter.last().transpose().unwrap(), None);

...
impl Env {
    ...
    /// Deregister the environment from the global environment table so that
    /// it can be closed later on, when dropped.
    ///
    /// Returns `None` if the environment was already closed, or a `Some(EnvClosingEvent)`
    /// handle that can be used to wait for the environment to be be closed from another thread.
    ///
